...
namespace TestCombinedProvider
{
    [TestClass]
    public class TestCombinedProvider
    {
        string connection = "Host = localhost; Username =postgres; Password =roman; Database =MIR";
        BalancePosition position = new BalancePosition("CH0317921671",
            FinType.Bond);

        [TestMethod]
        public void ReadDataFromMoexAndCbr()
        {
            ...
            DataProvider.Input.CombinedProvider.Provider combinedProvider =
                new DataProvider.Input.CombinedProvider.Provider(new List<IMarketProvider>() { MoexProvider, CbrProvider});

            var positionTS = combinedProvider.GetTimeSeries(position, TimeSeriesAttribute.Close);
            var currencyTS = combinedProvider.GetTimeSeries(Currencies.USD, TimeSeriesAttribute.Close);

            Assert.IsTrue(positionTS != null && positionTS.Series.Count != 0
                && currencyTS != null && currencyTS.Series.Count != 0);
            Console.WriteLine(positionTS.Series.Count);
            Console.WriteLine(currencyTS.Series.Count);
        }
    }
}
